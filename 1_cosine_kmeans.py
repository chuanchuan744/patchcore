...
def run_patchcore_pipeline():
    ...
    def save_visualizations(image_path, anomaly_map, save_base_path, dataset_root):
        ...
        if gt_mask is not None:
            Image.fromarray(compare_overlay).save(save_base_path + "_compare_gt_kmeans.png")
    ...
    def create_patchcore_model(pre_processor, evaluator):
        ...
        if DISTANCE_METRIC.lower() in ["cosine", "cos"]:
            ...
            print("警告: 当前 anomalib 版本的 Patchcore 初始化参数不支持 cosine 距离。")
            print("将自动回退到默认距离。默认通常是 euclidean。")
    ...
    try:
        ...
        print("开始保存热力图和 KMeans 分割结果...")
        save_vis_dir = os.path.join("results", "visualizations_cosine_kmeans")
        ...
        print(f"可视化保存完成，共保存 {saved_count} 组")
        print(f"保存目录: {os.path.abspath(save_vis_dir)}")

        kmeans_pixel_auroc = None
        kmeans_pixel_aupr = None

        if len(all_gt_masks) > 0:
            kmeans_pixel_auroc, kmeans_pixel_aupr = compute_kmeans_pixel_metrics(
                all_gt_masks,
                all_kmeans_scores,
            )
